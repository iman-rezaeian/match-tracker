...
function FieldCalibrationModal({ videoUrl, initialFieldName, initialLengthM, initialWidthM, onCancel, onSave }) {
  ...
  const [points, setPoints] = useState([]); // [{x, y}] in NATURAL video pixel coords
  ...
  // Load first frame onto canvas
  useEffect(() => {
    const v = videoRef.current;
    if (!v) return;
    v.crossOrigin = 'anonymous';
    const onLoaded = () => {
      // seek to t=0.5s to skip any black intro frame
    ...
    const onSeeked = () => {
      ...
      try {
        ...
        setLoadErr('Could not read video frame (CORS?). Try re-uploading the video.');
      }
    };
    const onErr = () => setLoadErr('Video failed to load. Check URL.');
  ...
  const handleClick = (e) => {
    ...
    // Scale display pixels back to natural video pixels.
    const sx = naturalSize.w / rect.width;
    const sy = naturalSize.h / rect.height;
    const x = Math.max(0, Math.min(naturalSize.w, cx * sx));
    const y = Math.max(0, Math.min(naturalSize.h, cy * sy));
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      src_points_px: src,                  // pixels in equirectangular frame
      dst_points_m: dst,
      homography: H,                       // 3x3 row-major
      video_frame_size: [naturalSize.w, naturalSize.h],
      created_at: Date.now(),
    };
    try {
      if (window.fbDb) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-stone-800">
          <h3 className="font-display text-base">🎯 FIELD CALIBRATION</h3>
          <button onClick={onCancel} className="text-stone-400 text-xs px-3 py-1 rounded-lg bg-stone-900 active:scale-95">CLOSE</button>
        </div>
        <div className="px-4 py-3 text-[11px] text-stone-300 border-b border-stone-800">
          {points.length < 4 ? (
            <>Tap the <strong className="text-lime-400">{CORNER_LABELS[points.length]}</strong> corner of the pitch. <span className="text-stone-500">{CORNER_HINTS[points.length]}</span></>
          ) : (
            <span className="text-lime-400">All 4 corners marked. Enter field name + dimensions below and SAVE.</span>
          )}
        </div>
        <div ref={containerRef} className="relative flex-1 overflow-auto bg-stone-900">
          {/* hidden source video used to grab the first frame onto the canvas */}
          <video
            ref={videoRef}
            src={videoUrl}
            muted
          ...
          />
          {/* Overlay points (positioned in % of natural size) */}
          {points.map((p, i) => {
            const left = naturalSize.w ? (p.x / naturalSize.w) * 100 : 0;
            const top = naturalSize.h ? (p.y / naturalSize.h) * 100 : 0;
            return (
              <div key={i} className="absolute pointer-events-none" style={{ left: `${left}%`, top: `${top}%`, transform: 'translate(-50%, -50%)' }}>
                <div className="w-3 h-3 rounded-full bg-lime-400 ring-2 ring-stone-950 shadow-lg" />
                <div className="text-[9px] font-bold text-lime-300 mt-0.5 text-center">{i + 1}</div>
              </div>
            );
          })}
          {!ready && !loadErr && (
            <div className="absolute inset-0 flex items-center justify-center text-stone-400 text-xs">Loading first frame…</div>
          )}
          {loadErr && (
            <div className="absolute inset-0 flex items-center justify-center text-red-400 text-xs px-6 text-center">{loadErr}</div>
          )}
        </div>
