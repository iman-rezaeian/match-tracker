...
// GK-only per-game context: OPP_GOALs conceded while this player was on the field,
// weighted penalty (gkFault flag), and whether they earned a clean sheet.
// Note: "on field" is used as a proxy for "in goal" — sufficient when only GK-tagged
// players are scored with these extras.
function gkExtrasForGame(playerId, game) {
  ...
  let onSince = starting ? game.startedAt : null;
  for (const sub of subs) {
    if (sub.playerId === playerId && onSince !== null) { intervals.push([onSince, sub.at]); onSince = null; }
  ...
  if (intervals.length === 0) return { oppGoalsConceded: 0, concededPenalty: 0, cleanSheets: 0, secondsOnField: 0 };
  let conceded = 0;
  let concededPenalty = 0; // raw -DEF points: 6 for GK fault, 0 for unstoppable, 3 for neutral/unknown
  for (const e of (game.events || [])) {
    if (e.type !== 'OPP_GOAL') continue;
    if (intervals.some(([s, en]) => e.at >= s && e.at <= en)) {
      conceded++;
  ...
  const secondsOnField = intervals.reduce((sum, [s, en]) => sum + Math.max(0, (en - s) / 1000), 0);
  const cleanSheets = (conceded === 0 && secondsOnField >= 60 && game.status === 'finished') ? 1 : 0;
  return { oppGoalsConceded: conceded, concededPenalty, cleanSheets, secondsOnField };
}
...
export default function App() {
  ...
  const startNewGame = (opponent, isHome, tournament, startingLineup) => {
    const now = Date.now();
    const game = {
      id: uid(),
    ...
    <div className="min-h-screen bg-stone-50 font-sans-pro text-stone-900">
      ...
      {view === 'lineup' && pendingGameSetup && (
        <StartingLineupView
          ...
          onStart={(lineup) => {
            startNewGame(pendingGameSetup.opponent, pendingGameSetup.isHome, pendingGameSetup.tournament, lineup);
          ...
          <div className="space-y-2">
            {sorted.map(p => (
              <div key={p.id} className="bg-white border border-stone-200 rounded-xl p-3 flex items-center gap-3">
                <div className="w-12 h-12 rounded-lg bg-stone-900 text-lime-400 flex items-center justify-center font-display text-2xl tabular-nums">
                  {p.number || '—'}
                </div>
                <div className="flex-1 min-w-0">
  ...
  return (
    <div className="pb-24">
      ...
      <div className="px-4 pt-6 space-y-5">
        ...
        <button
          onClick={() => valid && onSave({ ...player, name: name.trim(), number, position })}
          disabled={!valid}
  ...
  return (
    <div className="pb-32">
      ...
      <div className="px-4 pt-4">
        ...
        <div className="text-sm text-stone-700 mb-4">Tap to select players starting on the field. The rest go to the bench.</div>
          ...
          {sorted.map(p => {
            ...
            return (
              <button
                key={p.id}
                onClick={() => toggle(p.id)}
                className={`relative w-full flex items-center gap-3 p-2.5 rounded-xl border-2 transition active:scale-[0.98] ${
                  on
                    ? (isGK ? 'bg-amber-50 border-amber-400' : 'bg-lime-50 border-lime-400')
                    : (isGK ? 'bg-amber-50/50 border-amber-200 opacity-60' : 'bg-white border-stone-200 opacity-60')
                }`}
              >
                {isGK && (
                  <div className="absolute -top-2 -left-2 bg-amber-400 text-stone-900 text-[10px] font-extrabold tracking-wider px-1.5 py-0.5 rounded-full border border-amber-600 shadow-sm flex items-center gap-0.5">
                    <span>🧤</span><span>GK</span>
                  </div>
                )}
                <div className={`w-11 h-11 rounded-lg flex items-center justify-center font-display text-xl tabular-nums ${
                  on
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 p-4 shadow-xl">
        ...
        <button
          ...
          className="w-full bg-lime-500 text-stone-900 font-display text-2xl py-4 rounded-2xl shadow-lg shadow-lime-500/30 border-2 border-lime-600 active:scale-[0.99] transition"
        >
          ▶ START GAME
        </button>
      </div>
...
/* ---------- ACTIVE GAME ---------- */
function ActiveGameView({ game, roster, pendingEvent, onSelectEvent, onSelectPlayer, onResolveOppGoal, onCancelEvent, onUndo, onPauseHalfTime, onStartSecondHalf, onResumeFirstHalf, onEnd, onBack, tick }) {
  const elapsed = computeElapsed(game);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col px-4 pt-4">
        {pendingEvent?.type === 'OPP_GOAL_FAULT' ? (() => {
          ...
          const gkOnField = playersSorted.filter(p => p.position === 'GK' && onField.has(p.id));
          return (
            <div className="flex flex-col h-full min-h-0">
              ...
              {gkOnField.length === 0 && (
                <div className="mb-3 bg-stone-100 border border-stone-200 rounded-xl px-3 py-2 text-xs text-stone-600">
                  No GK-tagged player on the field. Pick a tag anyway — it'll be saved for record.
                </div>
              )}
          ...
          const gkFirst = (a, b) => {
            const ag = a.position === 'GK' ? 0 : 1;
            const bg = b.position === 'GK' ? 0 : 1;
          ...
          <>
            {quickMode ? (
              ...
              <>
              {/* 4x3 grid: 12 primary events */}
              <div className="grid grid-cols-4 gap-2">
                {['GOAL', 'ASSIST', 'KEY_PASS', 'BALL_WIN', 'SHOT_ON', 'SHOT_OFF', 'SAVE', 'BLOCK', 'DUEL_WIN', 'DUEL_LOSE', 'GIVE_GO', 'GATES'].map(id => {
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <button
                  ...
                  className={`${TONE_CLASSES['soft-red']} border-2 rounded-2xl py-3.5 flex items-center justify-center gap-2 active:scale-[0.97] transition`}
                >
                  <span className="text-xl">💨</span>
                  <span className="font-sans-pro font-extrabold tracking-tight text-base leading-none">TURNOVER</span>
                </button>
                <button
                  onClick={() => onSelectEvent('HOLDS_BALL')}
                  className={`${TONE_CLASSES['yellow']} border-2 rounded-2xl py-3.5 flex items-center justify-center gap-2 active:scale-[0.97] transition`}
                >
                  <span className="text-xl">⏳</span>
                  <span className="font-sans-pro font-extrabold tracking-tight text-base leading-none">HOLDS BALL</span>
                </button>
              </div>
            ...
            <button
              ...
              className={`mt-2.5 w-full ${TONE_CLASSES['purple']} border-2 rounded-2xl py-3.5 flex items-center justify-center gap-3 active:scale-[0.97] transition`}
            >
              <span className="text-2xl">🔄</span>
              <span className="font-sans-pro font-extrabold tracking-tight text-lg leading-none">SUBSTITUTION</span>
            </button>
...
function PlayerPicker({ event, players, skippable, onPick, onSkip, onUnknown, onCancel, emptyMessage }) {
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div>
        ...
        {players.map(p => {
          ...
          return (
            ...
            >
              ...
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center font-display text-2xl tabular-nums shrink-0 ${
                isGK ? 'bg-amber-500 text-stone-900' : 'bg-stone-900 text-lime-400'
              }`}>
                {p.number || '—'}
              </div>
          ...
          <div className="bg-white border border-stone-200 rounded-2xl divide-y divide-stone-100">
            {Object.entries(tally)
              .map(([pid, stats]) => {
                ...
                const pos = roster.find(p => p.id === pid)?.position;
                const gkExtras = pos === 'GK' ? gkExtrasForGame(pid, game) : undefined;
...
function StatsView({ roster, games, onBack }) {
  ...
  const seasonScores = useMemo(() => {
    ...
    for (const p of roster) {
      ...
      let gkExtras;
      if (p.position === 'GK') {
        gkExtras = { oppGoalsConceded: 0, cleanSheets: 0 };
        for (const g of finished) {
      ...
      map[p.id] = computePerformanceScore(p.id, allEvents, min, p.position, gkExtras);
    }
    return map;
            ...
            <div className="divide-y divide-stone-100">
              {sorted.map(p => {
                ...
                return (
                  ...
                  >
                    <div className="font-display text-lg tabular-nums text-stone-900">{p.number || '—'}</div>
