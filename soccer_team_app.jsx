...
function PublicAnalyticsCard({ game, roster: _roster }) {
  ...
  const broadcastDoc = {
    ...
    home_name: game?.broadcastHomeName || (game?.isHome ? 'Stompers' : (game?.opponent || 'OPP')),
    away_name: game?.broadcastAwayName || (game?.isHome ? (game?.opponent || 'OPP') : 'Stompers'),
...
function ScheduledScoreboard({ item }) {
  const isCancelled = !!item.cancelled;
        ...
        <>
          ...
          {featuredGame ? (
            <a href={`./?live=${featuredGame.id}`} className="block">
              <LiveScoreboard game={featuredGame} roster={roster} />
            </a>
          ) : (
            <ScheduledScoreboard item={featuredItem} />
          )}
