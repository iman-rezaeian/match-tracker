...
export default function App() {
  // Public read-only live scoreboard route. When the URL has ?live=<gameId>,
  // bypass the entire coach app and render a clean scoreboard for parents.
  // Computed once at mount; reading window.location here is safe because the
  // URL doesn't change without a reload.
  const liveGameId = (typeof window !== 'undefined')
  ...
  return (
    <div className="pb-24 bg-stone-50 min-h-screen">
      ...
      <div className="px-4 pt-3 space-y-2.5">
        ...
        <Section id="live" emoji="📡" title="12 · Live scoreboard for parents" summary="Share a read-only link — score & goals update in real time">
          <p>Open any past or in-progress game and tap the green <Pill tone="lime">📡 SHARE</Pill> button at the top. You'll get a link parents can open on any phone to see a Premier League-style scoreboard: live score, clock, and every goal with scorer + assist as they happen.</p>
          <p className="text-sm">Parents don't need to install anything, sign in, or have the app — just open the link. They <strong>only see the score and goals</strong>, never pillar events, weights, or rosters.</p>
          <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 mt-2 text-sm">
            <div className="font-display text-sm text-stone-700 mb-1">💡 Suggested workflow</div>
            <Step n={1}>Right after kickoff, open the active game from the home screen.</Step>
            <Step n={2}>Tap <Pill tone="lime">📡 SHARE</Pill> and send the link to your team's TeamSnap / WhatsApp chat once.</Step>
            <Step n={3}>Parents leave the tab open — it auto-updates every time you log a GOAL, ASSIST, or OPP GOAL. No refresh needed.</Step>
          </div>
          <p className="text-xs text-stone-500 mt-2">The link works for finished games too — useful for parents who couldn't make the match.</p>
        </Section>
...
/* =========================================================================
   ...
   Loaded via ?live=<gameId>. Premier-League-style scoreboard for parents:
   teams + score, live clock, chronological goal feed (scorer + assist).
   Subscribes to Firestore via window.fbDb so it auto-updates the moment the
   coach taps GOAL. Anonymous auth handles access; no rule changes needed.
   Shows ONLY score + goals — no pillar events, rosters, or weights.
   ========================================================================= */
    ...
    return (
      <div className="min-h-screen stripes-bg text-white flex items-center justify-center p-6">
        ...
        <div className="bg-white/10 rounded-2xl p-6 max-w-sm text-center">
          <div className="font-display text-3xl mb-2">⚠ {error}</div>
  ...
  if (!game) {
    return (
      <div className="min-h-screen stripes-bg text-white flex items-center justify-center">
        <style>{FONT_STYLES}</style>
        <div className="font-display text-2xl text-white/60">Loading live scoreboard…</div>
      </div>
    );
  }
