...
function GameDetail({ game, roster, weights, onBack, onDelete, onDeleteEvent, onUpdateEvent, onUpdateGame }) {
  ...
  const handleUpload = (file) => {
    ...
    fetch(`${R2_UPLOAD_WORKER}/upload-url`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ password: COACH_PASS, filename }),
    })
...
function LiveScoreboard({ game, roster }) {
  ...
  const nameSizeClass =
    longestName <= 8 ? 'text-2xl'
    : longestName <= 10 ? 'text-xl'
    : longestName <= 13 ? 'text-lg'
    : longestName <= 17 ? 'text-base'
    : 'text-sm';
      ...
      <div className="stripes-bg text-white px-4 pt-[calc(env(safe-area-inset-top,0px)+3.75rem)] pb-6">
        ...
        <div className="flex items-center justify-center gap-3 mt-4">
          <div className="flex-1 text-right min-w-0">
            <div className={`font-display ${nameSizeClass} leading-tight break-words`}>{leftName}</div>
          </div>
          <div className="font-display text-6xl tabular-nums px-3 shrink-0">
            {leftScore}<span className="text-white/40 mx-2">–</span>{rightScore}
          </div>
          <div className="flex-1 text-left min-w-0">
            <div className={`font-display ${nameSizeClass} leading-tight break-words`}>{rightName}</div>
          </div>
        </div>
