...
async function _voiceUpload(gameId, startedAt, blob, mime, durationS) {
  ...
  if (window.fbDb) {
    ...
    const cur = (snap.exists && snap.data().voiceSegments) || [];
    await ref.update({ voiceSegments: [...cur, { startedAt, url: publicUrl, durationS, mime }] });
  }
  return publicUrl;
}

function VoiceRecorder({ game }) {
  const [state, setState] = useState('idle'); // idle | rec | uploading | done | err
  const [secs, setSecs] = useState(0);
  const [armStop, setArmStop] = useState(false); // two-tap stop guard
  const recRef = useRef(null);

  // Recover orphaned chunks from a previous crashed session (best effort).
  useEffect(() => {
    (async () => {
      try {
        const rows = (await _voiceList()).filter(r => r.k.startsWith(`${game.id}|`));
        if (!rows.length) return;
        const bySeg = {};
        rows.forEach(r => {
          const [, seg] = r.k.split('|');
          (bySeg[seg] = bySeg[seg] || []).push(r);
        });
        for (const [seg, list] of Object.entries(bySeg)) {
          ...
          if (blob.size > 20000) {
            await _voiceUpload(game.id, Number(seg), blob, mime, Math.round(list.length * 10));
          }
          await _voiceClear(`${game.id}|${seg}|`);
        }
  ...
  if (state === 'rec') {
    return (
      <button
        onClick={() => {
          if (!armStop) { setArmStop(true); setTimeout(() => setArmStop(false), 2500); return; }
          setArmStop(false); stop();
        }}
        className={`shrink-0 rounded-full px-3 py-2.5 font-display text-xs tracking-widest border-2 active:scale-95 transition flex items-center gap-1 ${armStop ? 'bg-red-600 text-white border-red-400' : 'bg-red-500/15 text-red-300 border-red-600/60'}`}
        title="Recording — tap twice to stop"
      >
        <span className="inline-block w-2 h-2 rounded-full bg-red-500 animate-pulse" />
        <span>{armStop ? 'TAP TO STOP' : mmss}</span>
      </button>
    );
  }
  return (
    <button
      onClick={state === 'idle' || state === 'err' || state === 'done' ? start : undefined}
      ...
      title="Record sideline narration (synced to the game clock)"
    >
      <span>🎙</span>
