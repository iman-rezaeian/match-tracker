...
function FieldCalibrationModal({ videoUrl, initialFieldName, initialLengthM, initialWidthM, onCancel, onSave }) {
  ...
  // Seek to a frame past any black intro; show the video element directly.
  useEffect(() => {
    ...
    const onErr = () => setLoadErr('Video failed to load. Check URL or CORS settings on R2 bucket.');
  ...
  const handleClick = (e) => {
    ...
    const nw = v.videoWidth || naturalSize.w;
    const nh = v.videoHeight || naturalSize.h;
    if (!nw || !nh || !rect.width || !rect.height) return;
    const sx = nw / rect.width;
    const sy = nh / rect.height;
    const x = Math.max(0, Math.min(nw, cx * sx));
    const y = Math.max(0, Math.min(nh, cy * sy));
