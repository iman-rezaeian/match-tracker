...
function computePerformanceScore(playerId, events, minutesPlayed) {
  ...
  const overall = 0.30 * attacking + 0.25 * defending + 0.30 * decisions + 0.15 * involvement;
...
export default function App() {
  ...
  const logEvent = (gameId, eventType, playerId) => {
          ...
          {sorted.map(p => {
            ...
            return (
              <button
                key={p.id}
                onClick={() => toggle(p.id)}
                className={`w-full flex items-center gap-3 p-2.5 rounded-xl border-2 transition active:scale-[0.98] ${
                  on
              ...
              >
                <div className={`w-11 h-11 rounded-lg flex items-center justify-center font-display text-xl tabular-nums ${
                  on ? 'bg-stone-900 text-lime-400' : 'bg-stone-200 text-stone-500'
                }`}>
                  {p.number || '—'}
                </div>
...
/* ---------- ACTIVE GAME ---------- */
function ActiveGameView({ game, roster, pendingEvent, onSelectEvent, onSelectPlayer, onCancelEvent, onUndo, onPauseHalfTime, onStartSecondHalf, onResumeFirstHalf, onEnd, onBack, tick }) {
  const elapsed = computeElapsed(game);
        ...
        {pendingEvent ? (() => {
          ...
          if (isSub) {
            ...
            if (pendingEvent.step === 'OFF') {
              ...
              pickerPlayers = playersSorted.filter(p => onField.has(p.id));
            } else {
              pickerEvent = { emoji: '🔄', label: 'WHO\'S ON?', requiresPlayer: true };
              pickerPlayers = playersSorted.filter(p => !onField.has(p.id) && p.id !== pendingEvent.offPlayerId);
            }
            ...
            const onField = onFieldAt(game);
            pickerPlayers = playersSorted.filter(p => onField.has(p.id) && p.id !== pendingEvent.excludePlayerId);
      ...
      <div className="grid grid-cols-2 gap-2.5 pb-6 overflow-y-auto">
        ...
        {players.map(p => (
          <button
            key={p.id}
            onClick={() => onPick(p.id)}
            className="bg-white border-2 border-stone-200 hover:border-stone-900 rounded-xl p-3 flex items-center gap-3 active:scale-[0.97] transition text-left"
          >
            <div className="w-12 h-12 rounded-lg bg-stone-900 text-lime-400 flex items-center justify-center font-display text-2xl tabular-nums shrink-0">
              {p.number || '—'}
            </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {isSub ? (
          ...
          <>
            ...
            {!player && !ev.requiresPlayer && <div className="text-xs text-stone-500">No player</div>}
          </>
        )}
      </div>
          ...
          <div className="bg-white border border-stone-200 rounded-2xl divide-y divide-stone-100">
            {Object.entries(tally)
              .map(([pid, stats]) => {
                ...
                const score = computePerformanceScore(pid, events, min);
                return { pid, stats, min, score };
              })
...
function StatsView({ roster, games, onBack }) {
  ...
  const seasonScores = useMemo(() => {
    ...
    for (const p of roster) {
      ...
      map[p.id] = computePerformanceScore(p.id, allEvents, min);
    }
    return map;
