...
function computePerformanceScore(playerId, events, minutesPlayed, position, gkExtras = {}, weights) {
  ...
  const pts = isGK ? W.gkPoints : W.points;
  const attacking = (
...
function WeightsView({ weights, onSave, onBack }) {
  ...
  const [tab, setTab] = useState('outfield'); // 'outfield' | 'gk' | 'pillars'
  ...
  const PointsSection = ({ group }) => (
    <div className="space-y-4">
      ...
      <p className="text-xs text-stone-500 px-1">
        Points per action. Negative values penalize the score. {group === 'gkPoints' && 'Goalkeeper-only overrides — only used when the player is GK that game.'}
      </p>
    </div>
  );
  ...
  return (
    <div className="pb-32">
      ...
      <div className="px-4 pt-3 flex items-center gap-2">
        <TabBtn id="outfield" label="OUTFIELD" />
        <TabBtn id="gk"       label="GK" />
        <TabBtn id="pillars"  label="PILLARS" />
      </div>

      <div className="px-4 pt-4">
        {tab === 'outfield' && <PointsSection group="points" />}
        {tab === 'gk'       && <PointsSection group="gkPoints" />}
