...
function VideoPlayer360({ videoUrl, seekTo, onClose, events = [], gameInfo, dotsMode: initialDotsMode = 'all', lockDots = false }) {
  ...
  const playerNode = (
    <div ref={wrapperRef} style={wrapperStyle} className={`bg-black overflow-hidden ${isFullscreen ? '' : 'rounded-2xl border border-stone-800'}`}>
      ...
      {gameInfo && (() => {
        // Status text: half + minute (LIVE shown as a red blinking dot, not text).
        let statusLabel;
        ...
        return (
          <div className={`absolute z-10 pointer-events-none ${isFullscreen ? 'top-[max(env(safe-area-inset-top,0px)+10px,14px)] left-3' : 'top-2 left-2'}`}>
            <div
              className="rounded-2xl shadow-2xl border border-white/15 overflow-hidden backdrop-blur-md flex items-center"
              style={{ background: 'linear-gradient(135deg, rgba(15,15,18,0.92) 0%, rgba(28,28,32,0.88) 100%)' }}
            >
              {/* Live indicator — bigger blinking red dot on the left */}
              {showLiveDot && (
              ...
              <div className="flex items-center pl-1.5 pr-2.5 py-1.5">
                ...
                <span className="font-bold tracking-wider truncate max-w-[5rem] text-white text-[11px]" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.6)' }}>{(gameInfo.home || 'STM').toUpperCase()}</span>
              </div>
              {/* Score */}
              ...
              <div className="flex items-center pl-2.5 pr-1.5 py-1.5">
                <span className="font-bold tracking-wider truncate max-w-[5rem] text-white text-[11px]" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.6)' }}>{(gameInfo.away || 'OPP').toUpperCase()}</span>
              ...
              {/* Status — half + minute (or FT) */}
              <div className="pl-2 pr-3 py-1.5 flex items-center bg-black/30 border-l border-white/10">
                <span className="font-display tabular-nums text-white text-[12px] font-extrabold tracking-[0.12em] leading-none" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.7)' }}>{statusLabel}</span>
              </div>
            </div>
          </div>
        );
