...
new_persist = (
    ...
    "  };\n\n"
    "  const persistGames = async (nextOrFn) => {\n"
    "    // Resolve against the LATEST state, not a closed-over snapshot: two\n"
    "    // writes in one React batch (logEvent auto-chains GOAL -> ASSIST) would\n"
    "    // otherwise both start from the pre-batch games array and the second\n"
    "    // would discard the first — silently erasing a logged goal.\n"
    "    let prevList = null, next = null;\n"
    "    setGames(prev => {\n"
    "      prevList = prev;\n"
    "      next = typeof nextOrFn === 'function' ? nextOrFn(prev) : nextOrFn;\n"
    "      return next;\n"
    "    });\n"
    "    if (!next) return;\n"
