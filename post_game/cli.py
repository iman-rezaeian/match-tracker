...
def aim_diagnose(
    ...
    motion_model: str = typer.Option("smooth_damp", "--motion-model", help="smooth_damp (default) | kalman_deadzone | legacy_boxcar | learned"),
    no_kalman: bool = typer.Option(False, "--no-kalman", help="(kalman_deadzone model) Disable the Kalman predictive-lead stage."),
    no_dead_zone: bool = typer.Option(False, "--no-dead-zone", help="(kalman_deadzone model) Disable the dead-zone hysteresis stage."),
    no_events: bool = typer.Option(False, "--no-events", help="Disable event-aware FOV framing."),
    no_consensus_lead: bool = typer.Option(False, "--no-consensus-lead", help="Disable the consensus-velocity anticipation lead."),
    learned: bool = typer.Option(False, "--learned", help="Use the Holt/learned smooth predictor (overrides --motion-model)."),
    ...
    cfg = AimConfig(
        ...
        use_kalman=not no_kalman,
        use_dead_zone=not no_dead_zone,
        use_event_framing=not no_events,
        use_learned=learned,
        use_consensus_lead=not no_consensus_lead,
    ...
    console.print_json(json.dumps({
        ...
        "aim_mode": aim_mode,
        "motion_model": "learned" if learned else motion_model,
        "stages": {
            "event_framing": cfg.use_event_framing,
        },
